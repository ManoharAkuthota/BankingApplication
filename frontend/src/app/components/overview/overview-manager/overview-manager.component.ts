...
@Component({
    ...
    <div class="overview-container animate-fade-in">
      ...
      <div class="welcome-banner glass-panel">
        <div class="welcome-text">
          ...
          <p>Welcome back to APEX TRUST management system. Here is a summary of branch operations.</p>
        </div>
      ...
      <div class="quick-actions-grid">
        <a routerLink="/dashboard/accounts" class="action-card glass-panel">
          ...
          <div class="action-desc">
            <h4>Approve Accounts</h4>
            <p>Review and authorize pending accounts in this branch.</p>
          </div>
        </a>
        ...
        <a routerLink="/dashboard/users" class="action-card glass-panel">
          ...
          <div class="action-desc">
            ...
            <p>Add new employees under this branch.</p>
          </div>
        </a>
  ...
  styles: [`
    ...
    .welcome-banner {
      ...
      align-items: center;
      padding: 32px 40px;
      background: linear-gradient(135deg, rgba(6, 182, 212, 0.08), rgba(99, 102, 241, 0.03));
    ...
    .lookup-field:focus {
      ...
      box-shadow: 0 0 12px rgba(168, 85, 247, 0.255);
    }
    ...
    .action-icon.amber { background: rgba(245, 158, 11, 0.1); }
...
export class OverviewManagerComponent implements OnInit {
  ...
  loadDashboardData(branchId: number | null, showToast: boolean = false): void {
    this.dashboardService.getDashboardSummary('MANAGER', branchId, null).subscribe({
      next: (res) => {
        if (res.success && res.data) {
      ...
      error: (err) => {
        console.error('Error loading manager dashboard summary', err);
