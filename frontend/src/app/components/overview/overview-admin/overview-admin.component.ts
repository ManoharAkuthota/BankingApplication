    ...
    <div class="overview-container animate-fade-in">
      ...
      <div class="welcome-banner glass-panel">
        <div class="welcome-text">
          ...
          <p>Welcome back to APEX TRUST management system. Here is a summary of global operations.</p>
        </div>
      ...
      <div class="quick-actions-grid">
        <a routerLink="/dashboard/branches" class="action-card glass-panel">
          <span class="action-icon cyan">🏦</span>
          <div class="action-desc">
            <h4>Register Branch</h4>
            <p>Setup a new banking branch code, IFSC, and location.</p>
          </div>
        </a>
        ...
        <a routerLink="/dashboard/users" class="action-card glass-panel">
          ...
          <div class="action-desc">
            <h4>Register Managers & Staff</h4>
            <p>Create credentials for new managers and employees.</p>
          </div>
        </a>
      ...
      <!-- Branch Analysis Table (ADMIN ONLY) -->
      <div *ngIf="dashboardData?.branchSummaries?.length" class="branch-analysis-section">
        <div class="section-title">
        ...
        <div class="table-container glass-panel">
          <table class="data-table">
            ...
            <tbody>
              <tr *ngFor="let branch of dashboardData.branchSummaries">
                <td class="font-semibold">{{ branch.branchName }}</td>
                <td><span class="badge badge-code">{{ branch.branchCode }}</span></td>
                <td>{{ branch.activeUsersCount }}</td>
                <td>{{ branch.activeAccountsCount }}</td>
                <td class="text-emerald">₹{{ branch.totalBalance | number:'1.2-2' }}</td>
                <td class="text-indigo">₹{{ branch.totalDepositVolume | number:'1.2-2' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export class OverviewAdminComponent implements OnInit {
  ...
  loadDashboardData(): void {
    this.dashboardService.getDashboardSummary('ADMIN', null, null).subscribe({
      next: (res) => {
        if (res.success && res.data) {
      ...
      error: (err) => {
        ...
        const msg = err.error?.message || err.message || 'Failed to sync operational metrics.';
